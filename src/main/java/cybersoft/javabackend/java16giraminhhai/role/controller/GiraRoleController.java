...
import cybersoft.javabackend.java16giraminhhai.security.validator.ExistedUser;
...
public class GiraRoleController {
	...
	@GetMapping("/{role-id}")
	public Object findByRoleId (@PathVariable("role-id") String id) {
	...
	public Object createNewRole(@Valid @RequestBody GiraRoleDTO dto, 
		...
		GiraRoleDTO newRole = service.save(dto);
	...
	public Object updateRole(@PathVariable("role-id") String id,
		...
		GiraRole updateRole = service.update(UUID.fromString(id), dto);
		
		if(updateRole == null) {
			return ResponseHelper.getErrorResponse("Role code is used", HttpStatus.BAD_REQUEST);
		}
		
		return ResponseHelper.getResponse(updateRole, HttpStatus.OK);
	}
